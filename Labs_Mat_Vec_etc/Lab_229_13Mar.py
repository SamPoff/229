# -*- coding: utf-8 -*-
"""
Created on Mon Mar 13 15:56:29 2017
...
def neg(v):
    ...
    True
    """
    return Vec(v.D, {x: v[x] * -1 for x in v.D})
    pass

"""
u = Vec({1,2,3}, {1:1,2:2,3:5})
v = Vec({1,2,3}, {1:0,2:0,3:-3})
"""
...
"""       
# return Vec(veclist[0].D, {x: scalar_mul(veclist[0], coeflist[0])[x] + scalar_mul(veclist[1], coeflist[1])[x] + scalar_mul(veclist[2], coeflist[2])[x] for x in veclist[0].D})
def vecSum(veclist, D):
    sumv = Vec(D, {})
    for v in veclist:
        sumv = add(sumv, v)
    return sumv
print(vecSum([u,v,w], D))
"""
...
u = Vec(D, {'x':2, 'y':3})
v = Vec(D, {'x':1, 'y':2})
def linearCombo(veclist, coeflist): 
    return Vec(veclist[0].D, {x: scalar_mul(veclist[0],coeflist[0])[x]+scalar_mul(veclist[1],coeflist[1])[x] for x in veclist[0].D})
print(linearCombo([u,v], [2,-1]))

D = {1,2}
def stdVec(D):
    return [Vec(D, {x:1}) for x in D]
print('\n',stdVec(D))

R5D = {1,2,3,4,5}
print(linearCombo(stdVec(R5D), [1,-5,2]))
